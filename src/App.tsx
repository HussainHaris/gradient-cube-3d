...
import { Environment, RoundedBox, OrbitControls, shaderMaterial } from '@react-three/drei'
...
const GradientMaterial = shaderMaterial(
  {
    colorA: new THREE.Color('#ff6ec7'),
    colorB: new THREE.Color('#7ee8fa'),
    colorC: new THREE.Color('#eec0ff'),
    diffusionType: 0,
    gradientSharpness: 1,
    gradientOffset: 0,
    gradientRotation: 0,
  ...
  `
    ...
    void main() {
      vec3 gradientColor;
      vec3 rotatedPos = rotateVector(normalize(vPosition), gradientRotation);
      
      if (diffusionType < 0.5) {
        // Smooth linear gradient with sharpness control
        float t1 = (rotatedPos.x + rotatedPos.y + rotatedPos.z + 1.732) / 3.464 + gradientOffset;
        float t2 = (rotatedPos.x - rotatedPos.y + rotatedPos.z + 1.732) / 3.464 + gradientOffset * 0.5;
        
        // Apply sharpness - higher values create sharper transitions
        ...
        gradientColor = mix(colorA, colorB, clamp(t1, 0.0, 1.0));
        gradientColor = mix(gradientColor, colorC, clamp(t2, 0.0, 1.0));
      } else if (diffusionType < 1.5) {
        // Radial gradient with sharpness control
        float dist = length(rotatedPos.xy) * 0.8 + gradientOffset;
        float angle = atan(rotatedPos.y, rotatedPos.x) / 6.28318 + 0.5;
        
        // Apply sharpness to radial distance
        if (gradientSharpness > 1.0) {
        ...
        gradientColor = mix(colorA, colorB, clamp(dist, 0.0, 1.0));
        gradientColor = mix(gradientColor, colorC, clamp(angle, 0.0, 1.0));
      } else {
        // Sharp non-linear gradient with extreme sharpness
        float t1 = abs(rotatedPos.x + gradientOffset);
        float t2 = abs(rotatedPos.y + gradientOffset * 0.5);
        float t3 = abs(rotatedPos.z + gradientOffset * 0.3);
        
        // Create sharp color bands
        float band1 = smoothstep(0.3, 0.3 + 0.1 / gradientSharpness, t1);
        float band2 = smoothstep(0.6, 0.6 + 0.1 / gradientSharpness, t2);
        
        gradientColor = mix(colorA, colorB, band1);
        gradientColor = mix(gradientColor, colorC, band2);
      }
      
      // Add lighting
      vec3 lightDir = normalize(vec3(1.0, 1.0, 1.0));
      float diff = max(dot(vNormal, lightDir), 0.0);
      gradientColor *= (0.7 + 0.3 * diff);
